  ...
  <div class="layout">
    <Layout>
      <Sider ref="side1" hide-trigger collapsible :collapsed-width="80" v-model="isCollapsed" style="overflow-y: auto">
        <div class="layout-logo-left" :style="isCollapsed ? 'font-size:12px': ''">jasonzhi.cn</div>
        <Menu :accordion="true" width="auto" :class="menuitemClasses" @on-select="goLink" :active-name="$route.meta.module" :open-names="[$route.meta.group]" theme="dark" v-if="menus">
          <div class="" v-for="(item, index) in menus" :key="index">
            <Submenu :name="item.path" v-if="item.children && item.children.length > 0">
              ...
              <Menu-item :name="child.path" style="padding-left: 20px;" v-for="(child, ind) in item.children" :key="ind">
        ...
        <Content class="content-wrapper">
          <div class="layout-content">
            <div class="layout-content-main">
              ...
              <keep-alive>
                <router-view v-if="$route.meta.keep || ($route.meta.keep instanceof Array && $route.meta.keep.length > 0)" />
              </keep-alive>
              <router-view v-if="!$route.meta.keep || ($route.meta.keep instanceof Array && $route.meta.keep.length === 0)" />
            </div>
          </div>

<template>
  <section class="header">
    <header class="header-box">
      ...
      <div class="nav">
        <ul class="nav-box">
          <li v-for="(item, index) in nav" @click="changeNav(item.path, index)" :class="navIndex === index ? 'nav-item nav-item--active' : 'nav-item'" :key="index">
            <i :class="`icon ${item.icon}`"></i> {{item.name}}
          </li>
        </ul>
      </div>
      <div class="search">
        <el-input placeholder="请输入内容" size="small" clearable maxlength="32" prefix-icon="el-icon-search" @keyup.enter.native="getSearchArticle" v-model="keyword">
        </el-input>
      </div>
    </header>
  </section>
